  ...
  return (
    <section className="mt-6 rounded-[26px] border border-white/10 bg-white/[0.045] p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/35">Command brief</p>
          <h2 className="mt-1 text-lg font-semibold">What needs TJ now</h2>
        </div>
        <button
          type="button"
          onClick={() => void loadBrief()}
          className="rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-xs text-zinc-300"
        >
          Refresh
        </button>
      </div>
        ...
        <>
          <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-base font-semibold text-zinc-100">{brief.headline}</p>
